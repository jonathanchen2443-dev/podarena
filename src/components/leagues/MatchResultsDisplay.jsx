...
function PodiumLayout({ top3 }) {
  ...
  return (
    <div className="flex items-end justify-center gap-1 py-0 px-3 overflow-x-hidden">
      {second && (
        <PlayerCard p={second} imgSize={88}  blockH={Math.round(PODIUM_H * 0.75)} cardWidth={85} showCrown={false} />
      )}
      {first && (
        <PlayerCard p={first}  imgSize={104} blockH={PODIUM_H}                    cardWidth={85} showCrown />
      )}
      {third && (
        <PlayerCard p={third}  imgSize={76}  blockH={Math.round(PODIUM_H * 0.50)} cardWidth={85} showCrown={false} />
      )}
    </div>
  );
}
