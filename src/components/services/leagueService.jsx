...
export async function getLeagueStandings(auth, leagueId, inviteToken = null) {
  ...
  const activeMembers = allLeagueMembers.filter((m) => _profileIdSetForStandings.has(m.user_id));
...
export async function listLeagueMembers(auth, leagueId, inviteToken = null) {
  ...
  const _profileIdSetForMembers = new Set(_allProfilesForMembers.map((p) => p.id));
  const members = rawMembers.filter((m) => _profileIdSetForMembers.has(m.user_id));
